...
pub fn src_span_to_lsp_range(
    ...
    let end;
    if let Some(src) = src {
        start = line_numbers.line_and_column_number(location.start, src);
        end = line_numbers.line_and_column_number(location.end, src);
    } else {
        start = line_numbers.line_and_column_number_direct(location.start);
        end = line_numbers.line_and_column_number_direct(location.end);
    }
    Range::new(
